...
plt.scatter(data['longitude'].iloc[0], data['latitude'].iloc[0], color='green', label='起点', zorder=5, s=50)  # 起点，减小标记点大小
plt.scatter(data['longitude'].iloc[-1], data['latitude'].iloc[-1], color='red', label='终点', zorder=5, s=50)  # 终点
...
# 2. 绘制SOG图（线性）
plt.figure(figsize=(3, 2), dpi=600)  # 设置图片尺寸为3x2，DPI为600
plt.plot(data['base_date_time'], data['sog'], color='lightblue', label='SOG', linewidth=0.5, alpha=0.6)  # 更细的线条并增加透明度
...
# 3. 绘制COG图（线性）
plt.figure(figsize=(3, 2), dpi=600)  # 设置图片尺寸为3x2，DPI为600
plt.plot(data['base_date_time'], data['cog'], color='lightcoral', label='COG', linewidth=0.5, alpha=0.6)  # 更细的线条并增加透明度
plt.xlabel('时间', fontsize=7.5)
...
plt.axvline(mean_sog, color='red', linestyle='--', label=f'均值 SOG: {mean_sog:.2f} 节')  # 均值红线
plt.xlabel("地面速率 (SOG) [节]", fontsize=7.5)
