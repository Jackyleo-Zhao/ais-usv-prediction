# -*- coding: utf-8 -*-
"""
终稿（中文期刊排版增强版，最小改动口径修正版）
适用于 ais-USV_filled.csv

功能：
- 多预测时长：2 / 10 / 20 min
- 真实轨迹（实线） vs 预测轨迹（虚线） + 经验预测范围
- P50 典型片段 + P90 高误差片段
...
重要说明：
- 本脚本用于绘制第三章“经验预测范围表达”的示意图；
- 经验预测范围不等同于置信区间、置信域或概率风险边界；
- 图中文字不再使用“90%置信域”等表述。
"""
...
# 注册本地字体，确保 matplotlib / mathtext 能找到 Times New Roman
...
# 数学文本里的英文/数字/单位尽量走 Times New Roman
mpl.rcParams["mathtext.fontset"] = "custom"
mpl.rcParams["mathtext.rm"] = TIMES_NAME
mpl.rcParams["mathtext.it"] = f"{TIMES_NAME}:italic"
mpl.rcParams["mathtext.bf"] = f"{TIMES_NAME}:bold"
...
def add_mixed_title(ax, lab, tag, Hm, y=1.10):
    """
    子图标题中：
    (a)/(b)/(c)、P50/P90、2/10/20、min -> Times New Roman
    中文“预测时长=” -> 宋体
    """
    ax.set_title("")
    ...
    box = HPacker(
        children=[
            _ta(f"{lab} ", FP_TIMES, 11),
            _ta(f"{tag}", FP_TIMES, 11),
            _ta("，预测时长=", FP_SIMSUN, 11),
            _ta(f"{Hm} ", FP_TIMES, 11),
            _ta("min", FP_TIMES, 11),
        ],
        align="center",
        pad=0,
...
def add_mixed_ylabel_plain(ax, cn_text, en_text, x=-0.14, fontsize=11):
    """
...
def add_task3_top_legend(fig, alpha):
    """
    顶部图例：
    不再使用“置信域”或“90%”表述。
    """
    # 真实轨迹
    da1 = DrawingArea(22, 10, 0, 0)
    line1 = mpl.lines.Line2D([0, 20], [5, 5], color="C0", linestyle="-", linewidth=2.2)
    da1.add_artist(line1)
    item1 = HPacker(children=[da1, _ta("真实轨迹", FP_SIMSUN, 9)], align="center", pad=0, sep=3)

    # 预测轨迹
    da2 = DrawingArea(22, 10, 0, 0)
    line2 = mpl.lines.Line2D([0, 20], [5, 5], color="C1", linestyle="--", linewidth=1.6)
    da2.add_artist(line2)
    item2 = HPacker(children=[da2, _ta("预测轨迹", FP_SIMSUN, 9)], align="center", pad=0, sep=3)

    # 经验预测范围
    da3 = DrawingArea(22, 10, 0, 0)
    rect = Rectangle((0, 1), 20, 8, facecolor="C0", edgecolor="none", alpha=alpha)
    da3.add_artist(rect)
    item3 = HPacker(
        children=[
            da3,
            _ta("经验预测范围", FP_SIMSUN, 9),
        ],
        align="center",
        pad=0,
        sep=3
    ...
    topbox = HPacker(children=[item1, item2, item3], align="center", pad=0, sep=14)
...
def plot_task3(e, n, t_sec, v, theta, omega, sig_tbl, q1, q2,
    ...
    for ax, Hm, lab in zip(axes, HORIZONS_MIN, ["(a)", "(b)", "(c)"]):
        ...
        w = RANGE_SCALE * sigma * r
        dist = np.maximum(v0 * ts, 1e-3)
        w = np.minimum(w, FUNNEL_BETA * dist)
        w = np.minimum(w, W_SHORT_CAP[int(Hm)])

        ax.plot(e_gt, n_gt, "-", linewidth=2.2, zorder=4, label="真实轨迹")
        ax.plot(e_pr, n_pr, "--", linewidth=1.6, zorder=3, label="预测轨迹")
        ...
        add_mixed_title(ax, lab, tag, Hm, y=1.06)
        ax.set_xlabel("")
        ax.set_ylabel("")
        ...
        add_mixed_ylabel_plain(ax, "北向坐标/", "m", x=-0.14, fontsize=11)

        ax.title.set_fontproperties(FP_SIMSUN)
    ...
    if errs:
        ...
        add_mixed_ylabel_plain(ax_err, "误差/", "m", x=-0.08, fontsize=11)
    ...
    add_task3_top_legend(fig, FUNNEL_ALPHA)
...
def main():
    ...
    sig_tbl.to_csv(os.path.join(OUT_DIR, "任务3_经验范围参数表.csv"), index=False, encoding="utf-8-sig")
    ...
    plot_task3(
        e, n, t_sec, v_pos, th_pos, om_pos, sig_tbl, q1, q2,
        i_hard, tag="P90", stem="任务3_P90_高误差片段",
        v_cap=v_cap, omega_cap=omega_cap
    )
    ...
    print("  - 任务3_P90_高误差片段.pdf")
    print("  - 任务3_P90_高误差片段.png")
    print("  - 任务3_回测误差表.csv")
    print("  - 任务3_经验范围参数表.csv")
    print(f"AIS截断上限：速度上限={v_cap:.3f} m/s，对地航向变化率上限={omega_cap:.6f} rad/s")
    print(f"经验预测范围参数：alpha={FUNNEL_ALPHA}, stride={FUNNEL_STRIDE}, beta={FUNNEL_BETA}, "
          f"W_SHORT_CAP={W_SHORT_CAP}, grow_p={W_GROW_P}, range_scale={RANGE_SCALE}")
