...
def set_mixed_xlabel(ax, cn_text, en_text, y=-0.18, sep="/",
    ...
    box = HPacker(
        children=[
            ...
            TextArea(sep, textprops=dict(fontproperties=en_fp, fontsize=fontsize)),
...
def set_mixed_metric_title(ax, en_text, cn_text, y=1.02,
    ...
    """
    轴标题：英文/数字部分 Times New Roman，中文部分宋体
    例如：P50终点误差 / P90均方根误差
    """
...
def set_mixed_panel_title(ax, panel_tag, p_tag, horizon_min, y=1.04,
    ...
    子图标题：
    (a) P50，预测时长 = 2 min
    其中 (a)、P50、2、min 用 Times New Roman
    中文说明用宋体
    """
    _remove_mixed_artist(ax, "_mixed_title_artist")
    ax.set_title("")
...
def build_improvement_summary(bt_sub, subset_name, horizons, denom_eps=IMPROVE_DENOM_EPS):
    ...
    for H in horizons:
        hit = bt_sub[bt_sub["H_min"] == H].copy()
        for metric_name, ct_col, res_col in [("EPE 改善率 / %", "end_ct", "end_res"), ("RMSE 改善率 / %", "rmse_ct", "rmse_res")]:
...
def main():
    ...
    def plot_segment_compare(i0, tag):
        ...
        for j, H_min in enumerate(HORIZONS_MIN):
            ...
            ax = axs[j]
            _fill_funnel_polygon(
                ax, e_res, n_res, w, color=c_gt, alpha_max=0.40,
                label=("90% Prediction Region" if j == 0 else None)
            )

            ax.plot(
                e_gt, n_gt, color=c_gt, linewidth=2.0, zorder=6,
                label=("Ground Truth" if j == 0 else None)
            )
            ax.plot(
                e_cv, n_cv, color=c_cv, linestyle=":", linewidth=1.7, zorder=7,
                label=("CV-Heading" if j == 0 else None)
            )
            ax.plot(
                e_ct, n_ct, color=c_ct, linestyle="-.", linewidth=1.8, zorder=8,
                label=("CV-CT" if j == 0 else None)
            )
            ax.plot(
                e_res, n_res, color=c_res, linestyle="--", linewidth=1.8, zorder=9,
                label=("RAP-CT" if j == 0 else None)
            )

            ax.scatter([e_gt[0]], [n_gt[0]], s=28, marker="o", color="black", zorder=12)
            ax.scatter([e_gt[-1]], [n_gt[-1]], s=38, marker="^", color=c_gt, zorder=12)

            set_mixed_panel_title(
                ax,
                panel_tag=chr(97 + j),
                p_tag=tag,
                horizon_min=H_min,
            ...
            set_mixed_ylabel(ax, "北向坐标", "m", x=-0.20, fontsize=11)
        ...
        lg = fig.legend(
            handles, labels, loc="upper center", ncol=5, frameon=True,
            bbox_to_anchor=(0.5, 0.98)
        )
        apply_legend_font_times(lg, font=FP_TIMES)
        ...
        set_mixed_metric_title(ax1, f"{tag}", "终点误差", y=1.06, fontsize=12)
        ax1.set_xticks(x)
        ax1.set_xticklabels([str(int(h)) for h in hs])
        set_mixed_xlabel(ax1, "预测时长", "min", y=-0.20, fontsize=11)
        set_mixed_ylabel(ax1, "误差", "m", x=-0.17, fontsize=11)
        ax1.grid(True, axis="y", alpha=0.25)
        lg1 = ax1.legend(frameon=True, ncol=3, loc="upper left")
        apply_legend_font_times(lg1, font=FP_TIMES)
        ...
        set_mixed_metric_title(ax2, f"{tag}", "均方根误差", y=1.06, fontsize=12)
        ax2.set_xticks(x)
        ax2.set_xticklabels([str(int(h)) for h in hs])
        set_mixed_xlabel(ax2, "预测时长", "min", y=-0.20, fontsize=11)
        set_mixed_ylabel(ax2, "误差", "m", x=-0.17, fontsize=11)
        ax2.grid(True, axis="y", alpha=0.25)
        lg2 = ax2.legend(frameon=True, ncol=3, loc="upper left")
        apply_legend_font_times(lg2, font=FP_TIMES)
